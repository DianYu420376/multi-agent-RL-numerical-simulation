...
x_traj = x_history{i,1};
x_traj = x_traj(:,timeline);
x_traj = [xo_history(1)*ones(4,1),x_traj];
hold on
grid on
%plot(timeline, xo*ones(size(timeline)),'--')
plot(timeline1, xo_history(timeline2), 'k--','LineWidth',4)
plot(timeline1,x_set*ones(size(x_traj,2),1), 'b --','LineWidth',4)
plot(timeline1, x_traj', 'LineWidth',3)
set(gca,'FontSize',20)
...
hold off
saveas(a, ['figures/', num2str(i),'.eps'], 'epsc')
end
